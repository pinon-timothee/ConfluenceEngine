...
abstract class User implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\CE\\Model\\Map\\UserTableMap';
     ...
     *
     * Note: this column has a database default value of: 'en_GB'
     * @var        string
     */
    protected $culture;
    ...
    /**
     * Initializes internal state of CE\Model\Base\User object.
     * @see applyDefaults()
     */
    ...
    public function getLastLoginAt($format = NULL)
    {
        if ($format === null) {
            ...
            return $this->last_login_at instanceof \DateTimeInterface ? $this->last_login_at->format($format) : null;
        }
    }
     ...
     * @return $this|\CE\Model\User The current object (for fluent API support)
     */
    ...
    public function setCulture($v)
    {
        ...
        if ($this->culture !== $v) {
            $this->culture = $v;
            $this->modifiedColumns[UserTableMap::COL_CULTURE] = true;
        }
    ...
    public function setLastLoginAt($v)
    {
        ...
        if ($this->last_login_at !== null || $dt !== null) {
            if ($this->last_login_at === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->last_login_at->format("Y-m-d H:i:s.u")) {
                $this->last_login_at = $dt === null ? null : clone $dt;
                $this->modifiedColumns[UserTableMap::COL_LAST_LOGIN_AT] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UserTableMap::translateFieldName('Roles', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : UserTableMap::translateFieldName('Firstname', TableMap::TYPE_PHPNAME, $indexType)];
            $this->firstname = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 8 + $startcol : UserTableMap::translateFieldName('Culture', TableMap::TYPE_PHPNAME, $indexType)];
            $this->culture = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 9 + $startcol : UserTableMap::translateFieldName('Avatar', TableMap::TYPE_PHPNAME, $indexType)];
            $this->avatar = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 10 + $startcol : UserTableMap::translateFieldName('LastLoginAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->last_login_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;
            $this->resetModified();
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\CE\\Model\\User'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_CULTURE)) {
            $modifiedColumns[':p' . $index++]  = '`culture`';
        }
        if ($this->isColumnModified(UserTableMap::COL_AVATAR)) {
            $modifiedColumns[':p' . $index++]  = '`avatar`';
        }
        if ($this->isColumnModified(UserTableMap::COL_LAST_LOGIN_AT)) {
            $modifiedColumns[':p' . $index++]  = '`last_login_at`';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`avatar`':
                        ...
                        break;
                    case '`last_login_at`':
                        $stmt->bindValue($identifier, $this->last_login_at ? $this->last_login_at->format("Y-m-d H:i:s.u") : null, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[8], $arr)) {
            $this->setCulture($arr[$keys[8]]);
        }
        if (array_key_exists($keys[9], $arr)) {
            $this->setAvatar($arr[$keys[9]]);
        }
        if (array_key_exists($keys[10], $arr)) {
            $this->setLastLoginAt($arr[$keys[10]]);
        }
    }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\CE\Model\User The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_CULTURE)) {
            $criteria->add(UserTableMap::COL_CULTURE, $this->culture);
        }
        if ($this->isColumnModified(UserTableMap::COL_AVATAR)) {
            $criteria->add(UserTableMap::COL_AVATAR, $this->avatar);
        }
        if ($this->isColumnModified(UserTableMap::COL_LAST_LOGIN_AT)) {
            $criteria->add(UserTableMap::COL_LAST_LOGIN_AT, $this->last_login_at);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \CE\Model\User (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setLastLoginAt($this->getLastLoginAt());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \CE\Model\User Clone of current object.
     * @throws PropelException
     */
    ...
    public function setUserWebsites(Collection $userWebsites, ConnectionInterface $con = null)
    {
        ...
        foreach ($userWebsitesToDelete as $userWebsiteRemoved) {
            $userWebsiteRemoved->setUser(null);
        }
     ...
     * @return ObjectCollection|ChildUserWebsite[] List of ChildUserWebsite objects
     */
    public function getUserWebsitesJoinWebsite(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildUserWebsiteQuery::create(null, $criteria);
